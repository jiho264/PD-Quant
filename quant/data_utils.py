...
def save_dc_fp_data(
    ...
    get_inp_out = GetDcFpLayerInpOut(
        model, layer, device=device, input_prob=input_prob, lamb=lamb, bn_lr=bn_lr
    )
    cached_batches = []

    print(
        "Start correcting {} batches of data!".format(
            int(cali_data.size(0) / batch_size)
        )
    )
    for i in trange(int(cali_data.size(0) / batch_size)):
        if input_prob:
            cur_out, out_fp, cur_sym = get_inp_out(
                cali_data[i * batch_size : (i + 1) * batch_size]
            )
            cached_batches.append((cur_out.cpu(), out_fp.cpu(), cur_sym.cpu()))
        else:
            cur_out, out_fp = get_inp_out(
                cali_data[i * batch_size : (i + 1) * batch_size]
            )
...
class GetLayerInpOut:
    def __init__(
        ...
        self.data_saver = DataSaverHook(
            store_input=True, store_output=False, stop_forward=True
        )
...
class GetDcFpLayerInpOut:
    def __init__(
        ...
        self.device = device
        self.data_saver = DataSaverHook(
            store_input=True, store_output=True, stop_forward=False
        )
        self.input_prob = input_prob
        self.bn_stats = []
        self.eps = 1e-6
            ...
            if isinstance(m, nn.BatchNorm2d):
                ...
                self.bn_stats.append(
                    (
                        ...
                        torch.sqrt(m.running_var + self.eps)
                        .detach()
                        .clone()
                        .flatten()
                        .cuda(),
