...
def find_unquantized_module(
    model: torch.nn.Module, module_list: list = [], name_list: list = []
):
...
def block_reconstruction(
    ...
    if fromCheckPoint != True:
        ...
        set_act_quantize_params(
            block, cali_data=cached_inps[: min(256, cached_inps.size(0))]
        )
    ...
    for module in block.modules():
        ...
        if isinstance(module, (QuantModule, BaseQuantBlock)):
            if module.act_quantizer.delta is not None:
                module.act_quantizer.delta = torch.nn.Parameter(
                    torch.tensor(module.act_quantizer.delta)
                )
    ...
    for i in range(iters):
        ...
        if input_prob < 1.0:
            drop_inp = torch.where(
                torch.rand_like(cur_inp) < input_prob, cur_inp, cur_sym
            )
...
class LossFunction:
    ...
    def __call__(self, pred, tgt, output, output_fp):
        ...
        # pd_loss = s elf.JensenShannonDivLoss(output, output_fp)
        pd_loss = (
            self.pd_loss(
        ...
        if self.count % 500 == 0:
            print(
                "Total loss:\t{:.3f} (rec:{:.3f}, pd:{:.3f}, round:{:.3f})\tb={:.2f}\tcount={}".format(
                    float(total_loss),
                    float(rec_loss),
                    float(pd_loss),
                    float(round_loss),
