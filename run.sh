...
log_file="logs/resnet18/W${n_bits_w}A${n_bits_a}_calib${num_samples}_batch${batch_size}_iterw${iters_w}_tmp.log"
# Clear the log file
> $log_file

echo "General parameters for data and model" > $log_file
echo "- seed = $seed (default = 1005)" >> $log_file
echo "- arch = $arch (default = 'resnet18', choices = ['resnet18', 'resnet50', 'mobilenetv2', 'regnetx_600m', 'regnetx_3200m', 'mnasnet'])" >> $log_file
echo "- batch_size = $batch_size (default = 64)" >> $log_file
echo "- workers = $workers (default = 4)" >> $log_file
echo "- data_path = $data_path (default = '/datasets-to-imagenet')" >> $log_file
echo "" >> $log_file
# Quantization parameters
